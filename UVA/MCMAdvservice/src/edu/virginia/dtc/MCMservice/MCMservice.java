...
import edu.virginia.dtc.SItracking.SItracking_processing;
import edu.virginia.dtc.SItracking.IOB_tracking;
...
public class MCMservice extends Service
{
	...
	************************************************************************************/
	    	...
	    	case Meal.MEAL_ACTIVITY_CALCULATES_BOLUS_PUMP_MODE:
	            switch(DIAS_STATE) 
	            {
					...
					case State.DIAS_STATE_CLOSED_LOOP:
						closedLoopCalculation(bg, carbs, corr,subject_data);
						break;
	            }
	    		break;
	...
	private void closedLoopCalculation(double bg, double carbs, double corr, Subject subject_data)
	{
		...
		Context context = getApplicationContext();
		subject_data.read(System.currentTimeMillis(),context); // get subject parameters BW,TDI,BASAL,etc. see Subject.java
		...
		double total = 0.0;
		double CHO_insulin=0.0;
		double CORR_insulin=0.0;
		double IOB_total=0.0;
		double IOB_basal=0.0;
		long time = (System.currentTimeMillis()/1000);
		double BW= subject_data.subjectWeight;
		double IOBbasal=0;
		
		SItracking_processing SItrack= new SItracking_processing(context,time,BW);// get SI average of last 6h
		
		double SImedian=0.0; //collect median SI for user
		double SI25pc=0.0; //collect 25th percentile SI for user
		double SI75pc=0.0; //collect 75th percentile SI for user
		double SIfactor=1; //default multiplicative factor
		...
		//compute total and basal IOB based on updated Swan Action curves
