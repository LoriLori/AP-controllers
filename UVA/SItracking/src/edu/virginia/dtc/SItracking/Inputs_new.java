...
public class Inputs_new {
	
  private static final boolean DEBUG_MODE = true;
  public final String TAG = "SItracking_inputs";
  ...
  public Inputs_new(long length, long interval, double Gb) {
  ...
  public void reconstruct (long time,Context callingContext) {
	  ...
	  if (indicesCGM1 == null) {
		  for(i=0;i<steps;i++){
			  CGM_array[i]=Gb;// don't need other operations?
	  ...
	  //**************************construct J_dev and CGM_log*****************************//
	  
	  for (i=0;i<steps;i++) {
		  J_dev[i] = J[i] - basal[i];
		  CGM_log[i] = Math.log(CGM[i]/ Gb);
	  }
	...
	try {
		...
		if (c.moveToFirst()) {
			do {
				...
				Tvec_insulin.put(c.getLong(c.getColumnIndex("deliv_time")), insulin_value*1000); // sec mU
				}
			 } while(c.moveToNext());		    
		}	
		c.close();
